...
class TestGetConfigValue:
    def test_get_config_value_defaults_1(self, read_config_mock: Mock):
        read_config_mock.return_value = {"DEFAULT": {}}
...
class TestReadConfig:
    def test_read_config_1(
        self, get_app_paths_mock: Mock, config_read_mock: Mock, path_exists_mock: Mock
    ):
        path_exists_mock.return_value = True
        get_app_paths_mock.side_effect = (
            lambda global_: SimpleNamespace(config_path="path_global")
            if global_
            else SimpleNamespace(config_path="path_local")
        )
        read_config(None)
        path_exists_mock.assert_called_with("path_global")
        get_app_paths_mock.assert_has_calls([call(True), call(False)])
        config_read_mock.assert_called_once_with(["path_global", "path_local"])
    ...
    def test_read_config_2(
        ...
        config_read_mock.assert_called_once_with(["path_global"])
    ...
    def test_read_config_3(
        ...
        config_read_mock.assert_called_once_with(["path_local"])
...
class TestSaveConfig:
    def test_save_config_1(self, get_app_paths_mock: Mock, open_mock: Mock):
        config_mock = SimpleNamespace(write=Mock())
        get_app_paths_mock.return_value = SimpleNamespace(config_path="config-path")
    ...
    def test_save_config_2(self, get_app_paths_mock: Mock, open_mock: Mock):
        config_mock = SimpleNamespace(write=Mock())
        get_app_paths_mock.return_value = SimpleNamespace(config_path="config-path-2")
    ...
    def test_save_config_3(self, get_app_paths_mock: Mock, open_mock: Mock):
        config_mock = SimpleNamespace(write=Mock())
        get_app_paths_mock.return_value = SimpleNamespace(config_path="config-path-3")
